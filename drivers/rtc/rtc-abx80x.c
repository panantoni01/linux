			     ...
			     void *val, size_t bytes, bool write)
{
	...
	while (bytes) {
		...
		ret = regmap_write(priv->regmap, ABX8XX_REG_EXTRAM, extram);
		if (ret)
			return ret;
...
static const struct regmap_range abx80x_no_write_ranges[] = {
	regmap_reg_range(ABX8XX_REG_STATUS, ABX8XX_REG_STATUS),
	regmap_reg_range(0x1e, 0x1e),
	regmap_reg_range(0x22, 0x25),
	regmap_reg_range(ABX8XX_REG_ID0, ABX8XX_REG_ID0 + 7),
	regmap_reg_range(0x31, 0x3e),
};
...
static const struct regmap_config abx80x_regmap_config = {
	.reg_bits = 8,
	.val_bits = 8,
			...
			struct device_node *np, unsigned int part)
{
	...
	if (part == RV1805) {
		...
		if (err < 0) {
			dev_err(dev, "Unable to read control2 register\n");
			return -EIO;
		}
...
static int abx80x_i2c_probe(struct i2c_client *client)
{
	...
	regmap = devm_regmap_init_i2c(client, &abx80x_regmap_config);
	if (IS_ERR(regmap)) {
	...
	return abx80x_probe(&client->dev, regmap, client->irq,
			    client->dev.of_node, id->driver_data);
}
...
static int abx81x_spi_probe(struct spi_device *spi)
{
	...
	regmap = devm_regmap_init_spi(spi, &abx80x_regmap_config);
	if (IS_ERR(regmap)) {
	...
	return abx80x_probe(&spi->dev, regmap, spi->irq,
			    spi->dev.of_node, id->driver_data);
}
